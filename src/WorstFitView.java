...
public class WorstFitView extends BorderPane {
	...
	public WorstFitView() throws FileNotFoundException {
...
//				autoScrollToEnd(mainScrollP);
	...
	private PCB insertFromJobQ() {
		for(PCB process: jobQ) {
			Hole hole = findWorstFit(process);
			if(hole!=null) {
				jobQ.remove(process);
				holes.remove(hole);
	...
	private Hole findWorstFit(PCB process) { // worst fit algorithm
		int max = Integer.MIN_VALUE;
		Hole maxHole = null;
		for(Hole hole: holes) {
			if(process.getProcessSize() <= hole.getHoleSize() && hole.getHoleSize() > max) {
				max = hole.getHoleSize();
				maxHole = hole;
			}
		}
		return maxHole;
